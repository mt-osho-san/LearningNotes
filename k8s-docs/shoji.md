...
# スケジューリング、プリエンプション、退避

- スケジューリングとは、稼働させたい Pod をノードにマッチさせ、kubelet が実行できるようにすること
- プリエンプションは、優先度の低い Pod を終了させて、より優先度の高い Pod がノード上でスケジュールできるようにするプロセス
- 退避とは、リソース不足のノードで 1 つ以上の Pod を積極的に終了させるプロセス

# k8s のスケジューラー

- スケジューラーは新規に作成されて、Node に割り当てられていない Pod を監視し、どの Node で稼働させるかを見つけ出す
- kube-scheduler は k8s のデフォルトのスケジューラーで、コントロールプレーンの一部
- Pod や各コンテナにはそれぞれリソース要件があり、それらの要件にあうものを割り当てる (フィルタリング)
- スケジューラーは Pod に対する割当可能な Node を見つけ、スコアつけ、最もスコアの高い Node に割り当てる (スコアリング)

# ノード上への Pod のスケジューリング

- Pod を特定のノードで実行するように 制限 したり、特定のノードで実行することを 優先 させたりといった制約をかけることができる
- これを実現するための方法はいくつかあるが、推奨されている方法はラベルセレクターを利用する方法
- 他のオブジェクト同様、ノードにもラベルがあり、標準ラベルに加え、手動でラベル付ができる。
- アフィニティとアンチアフィニティ
  - アフィニティは Pod を特定の条件を満たすノードに 優先的に配置 するためのルール
  - アンチアフィニティは、特定のノードや他の Pod が存在するノードに Pod を配置しないようにする ルール
  - required と preferred のように制限のゆるさを変えることができる
  - weight により、重みを指定できる
- nodeName により、直接ノード名をしていして直接スケジューリングできる

# Pod のオーバーヘッド

- Pod のオーバーヘッドは、コンテナの要求と制限に加えて、Pod のインフラストラクチャで消費されるリソースを計算するための機能
- システム管理リソースやコンテナランタイムのリソースなどのオーバーヘッドを考慮しないと、スケジューリングの際に ノードがリソース不足になる可能性がある ため、Pod のオーバーヘッドを管理する仕組みが導入

# Pod トポロジー分散制約

- Pod の配置をノードやゾーンなどのトポロジー単位で均等に分散させるための機能
- この制約により 2 つだけ Pod があるときに、1 つのノードにどちらもあると、ノード障害でシステムが止まってしまうようなことを避けることができる
- topologySpreadConstraints エントリのフィールドにより制御できる
  - maxSkew は Pod が不均等に分散される程度
  - minDomains は特定のトポロジーのインスンタンスの最小数
  - など
- グループ内のすべての Pod に同じトポロジー分散制約が適用されておらず、ことなる制約が混在すると、トラブルシューティングが困難になるので、一貫性が重要
  - 複数の制約により競合が発生すると、Pod が配置できないこともある

# Pod のスケジューリング準備

- スケジューラーは、すべての保留中の Pod をできるだけ早く適切なノードにスケジュールしようとする
- しかし、下記のような場合は意図的にスケジューリングを遅らせたほうが良い
  - 必要なリソースが揃っていない Pod のスケジューリングを防ぐ: 依存関係が未作成のまま Pod が作成されると、Pod は Pending となり、無駄にリソースを消費することになる
  - Cluster AutoScaler などの誤作動を防ぐ: 未準備の Pod が多くあると、ノード不足と誤認し、不必要に新しいノードを追加することがある
- 上記を避けるために、スケジューリングされる条件を変更できる

# Taint と Toleration

- Taint
  - ノードに適用される属性であり、「このノードには特定の条件を満たす Pod 以外はスケジューリングさせない」というルールを設定できる
- Toleration
  - Pod に適用される属性であり、特定の Taint を持つノードにもスケジューリングされることを許可するための設定
- 下記は taint の例で、node1 には Pod に合致する toleration がなければスケジューリングされない。
  kubectl taint nodes node1 key1=value1:NoSchedule
- 下記 toleration があれば、node1 へスケジューリングされる
...
- ユースケース
  - 専有ノード：あるノード群を特定のユーザーに専有させたい場合
  - 特殊なハードウェアを備えるノード：クラスターの中の少数のノードが特殊なハードウェア（例えば GPU）を備える場合、そのハードウェアを必要としない Pod がスケジューリングされないようにして、後でハードウェアを必要とする Pod ができたときの余裕を確保

# スケジューリングフレームワーク

- スケジューラーに対してプラグイン可能なアーキテクチャであり、スケジューラーを拡張・カスタマイズするための仕組み
- Scheduling Cycle と Binding Cycle
  - Scheduling cycle は Pod が稼働する Node を決定、Pod に対して 1 つ 1 つ順番に実行される
  - Binding Cycle は上記 scheduling をクラスターに適用していき、これは並列に実行される
- 拡張点の例
  - QueueSort:Pod のスケジュール順を決める
  - PreFilter: Pod に関する情報を前処理したり、クラスターや Pod が満たすべき特定の条件をチェックするために使用
  - Filter: Pod を実行できない Node を候補から除外
  - PreScore: Score プラグインが使用する共有可能な状態を生成する「スコアリングの事前」作業を行うために使用
  - Score: フィルタリングのフェーズを通過した Node をランク付けするために使用
  - Reserve : 一時的にリソースを予約

# 動的リソース割り当て

- Pod と Pod 内のコンテナ間でリソースを要求および共有するための API
- 利用前に明示的な有効化および、特定のリソースのリソースドライバーのインストールも必要
- API
  - ResourceClaim: ワークロードによって使用される、クラスター内のリソースへのアクセス要求を記述
  - ResourceClaimTemplate: ResourceClaim を作成するための仕様とメタデータを定義
  - DeviceClass: 特定のデバイスとそれらの構成に対する事前定義の選択基準が含まれている
  - PodSchedulingContext: Pod のスケジューリングを調整するために、コントロールプレーンとリソースドライバーによって内部的に使用
  - ResourceSlice: クラスター内で使用可能なリソースに関する情報を公開するために、構造化パラメーターとともに使用

# スケジューラーのパフォーマンスチューニング

- kube-scheduler は、クラスター内のノードに Pod を適切に割り当てる
- 特に大規模クラスターでは、スケジューリングの速度と精度 のバランスを調整するため、パフォーマンスチューニングが必要
- ノードへのスコア付けの閾値により、割り当て可能なノードが十分に見つかるとノードの検索を停止することで、すべてのノードを考慮する単純なアプローチと比較して時間を節約
- クラスター内のすべてのノードに対する十分なノード数を整数パーセンテージで指定
- 閾値を指定しない場合、100 ノードのクラスターでは 50%、5000 ノードのクラスターでは 10%になる線形方程式を使用して数値を計算

# 拡張リソースのリソースビンパッキング

- ノードのリソースを効率的に活用するために、リソース使用量が最適化されるように Pod をスケジューリングする手法
- RequestedToCapacityRatioResourceAllocation により、ノードのリソース使用率 (Requested ÷ Capacity) に応じてスコアを計算し、最もリソースを活用できるノードに Pod をスケジューリング
- 優先度関数のチューニング :shape では、utilization が 0%の場合は 0、100%の場合は 10 という score を Node に与える

# Pod の優先度とプリエンプション

- Pod は priority を持つことができ、優先度は他の Pod に対する相対的な Pod の重要度を示す
- もし Pod をスケジューリングできないときには、スケジューラーはその Pod をスケジューリングできるようにするため、優先度の低い Pod をプリエンプトすることを試みる
- 優先度とプリエンプションを使うには、
  - 1 つまたは複数の PriorityClass を追加
  - 追加した PriorityClass を priorityClassName に設定した Pod を作成

# ノードの圧迫による退避

- ノード圧迫による退避は、kubelet がノード上のリソースを回収するために Pod を積極的に失敗させるプロセス
- 退避のトリガー:kubelet はノード上のリソース使用状況を監視し、事前に設定された閾値を超えた場合、リソース回収のために Pod を終了させる
- 自己修復の仕組み :Pod を終了させる前に、kubelet は未使用のコンテナイメージの削除など、ノードレベルでのリソース回収を試みる
- 退避シグナルと閾値:kubelet は特定のリソース状態を監視し、設定された退避閾値と比較して退避の必要性を判断

# API を起点とした退避

- Eviction API を使用して退避オブジェクトを作成し、Pod の正常終了を起動させるプロセス
- Eviction API: Pod を安全に退避（Eviction） させるための API
- Eviction API を直接呼び出すか、kubectl drain コマンドのように API サーバーのクライアントを使って退避を要求することが可能
