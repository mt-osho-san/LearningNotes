...
- いろんな歴史の話
- いろんな組織があるんだな〜という感じ
- MDN が Mozilla ってのは知っていたけど、Google や MS もコントリビュートしているのは知らなかった 事実上のブラウザの標準みたい
- HTTP/0.9 では HTML 以外を送る想定がなかったり、ステータスを返すこともできなかった
- ヘッダーに X をつけると独自ヘッダーとして定義できるのは知らなかった X-xxxx みたいなやつはよく見かけていたから理由がよくわかった ヘッダーの Key 値はライブラリによって正規化の方法が変わるのはなんとなくそんな気がしてたけど実際に知れてよかった
...
## 12:JavaScript によるブラウザからの動的な HTTP リクエスト

- SSE の再接続時には最後に受信した ID を Last-Event-ID ヘッダーフィールドとして送信すると、サーバー側がそれ以降のイベントのみを送信するのは知らなかった
- EventSource で認証情報とか送れないのであれば Cookie 以外の認証は EventSource では難しいのかな？
- WebSocket の開始は必ずクライアントからだが、通信を繋げられたら双方向で通信ができるのは便利
- Node.js で fetch が普通に使えるようになっていたの知らなかった
- JavaScript からブラウザのリロードを伴う HTTP アクセスは何かしらの場面で必要になることがあるかも
  - SPA だとないのかな？
- data uri のおかげで js でデータを生成して任意のデータをダウンロードさせられるのは便利

## 13: ウェブアプリケーションの知識

- Ajax って XMLHttpRequest でリクエストするっていう企画があるの?
  - 個人的には js でサーバーサイドからデータを取ってくることの総称だと思ってたけど、fetch を使う場合は Ajax ではなくなる？
- SPA の前はサーバーサイドで HTML を加工する、SPA は HTML を生成して DOM 操作なので、js ファイルだけ読み込むようになってきた
  - わかるようなわからんような
- HTML に関するデータをサーバーとネゴる必要がなくなったから、いつでも瞬時に見栄えの変更をできる感じか
- Next とか Nuxt はサーバーサイドレンダリングを簡単にしてくれるらしい
- ISR は初めて知った。SSG よりも生成する部分が少ないのか？何が違うの？
  - GPT 曰く、キャッシュ期間や再生成のタイミングで動的に更新を行うみたい
- RSC はなんで HTML じゃなくて JSON を返す？HTML ごと返せないの？あと、他のものと違いがわからん
  - クライアントに送る JS の量を減らすことができるみたい
  - その分サーバーサイドで JS を実行させるとのこと
  - リアルタイム処理もサーバーでさせておいてクライアントに返す JS を少なくするとのこと
  - おそらく、サーバー側で動かす useState みたいなことができるのかも
    - 確かに client 側で動くのか server 側で動くのか use 関数を指定した覚えがある
  - 逆にいうと、server 側で動かそうと思ったら、client 側では動かない説ある
- ホットモジュールリプレースメントは初めて知った。これは開発捗りそう
- GIL のせいでプロセスの多重化や複数プロセスとの接続のための interface が必要になるのは面倒くさいなという感じ
  - WSGI とか uvicorn とか聞いたことあるから、何で必要なのかなんとなくわかってよかった
  - ASGI っていう非同期の interface もあるらしい
- CGI どの言語でも簡単にレスポンス返せるやんと思ったけど、いちいちプロセス生成するのは確かにもう無理やろなと思った

## 14: クラウド時代の HTTP:ウェブを強くする様々な技術

- DNS でのサービスディスカバリはなんで A レコードじゃなくて SRV レコード？
  - port まで指定したいから？
- mDNS ってのでローカル DNS なしにマルチキャストで名前解決できるってのは知らなかった
- DNS over HTTPS は HTTPS によって DNS クエリするみたいで、HTTPS の恩恵を受けれるとのことだけど、送信先の中身まで秘匿化したいもの？
  - もう IP アドレスは機微情報ではなく公開されているものって感じはするけど
- nginx は API GW としてもロードバランサーとしてもベースになるような技術みたい

  - nginx に興味湧いたかも

- リバプロ自作が良い時もあるかもだけど、組織で共有しとかないと、パフォーマンスとか意図しないヘッダー書き換えとか経路書き換えになるから注意必要
- Cache-Status;hit でキャッシュしたかどうかわかるのは便利
- private ディレクティブがないと、CDN にキャッシュされてしまい、ユーザー毎に変わるデータであってもキャッシュしてしまうので、これには注意とのこと
- 複数の CDN を併用する場合などに設定のミスがあるとループしてしまうとのことだが、どうやったらループするのか？
- そのための CDN-Loop ヘッダーフィールドがあるらしい
- 接続ドレインは知っておいた方が良さそうだが、そもそもステートレスにしておけば OK そうな気もする
- LivenessProbe と ReadinessProbe はいつもどっちがどっちかなのか迷う
  - ReadinessProbe はリッチな気がするけど、この仕組みを自作しないといけないのかな？

> そのため、フロント側のサーバーとブラウザ間では BFF と呼ばれるランダムな認証トークンを利用 し、その BFF で JWT トークンを発行して、後段のマイクロサービスには JWT を送付するような組み合 わせが利用されることもあります。

- 上の文章わけわからん。BFF はアーキテクチャパターンでは？
- Jaeger って Uber が作ったやつなんだ。Istio と一緒に使える
- Otel ってスパン毎に一回一回データを送信しているの知らなかった
- Trace Context という規格ができそうなの知らなかった
- もう一回オブザーバビリティエンジニアリング読み直して、お家 k8s で実装するのありだな〜
- Devtool でサーバー側の処理時間も確認できるのすごい
  - ただ、一般公開して良いかはセキュリティ観点からよくわからん
  - 開発者だけが見ることができるようにすることとかってできるのかな？
    - 認証認可頑張るしかない？
...
- OpenSocial よくわからん。。。
  - 結局どういうこと？
- スーパアプリも何となくわかったけど、よくわからん
  - VSCode はスーパアプリ？
    - GPT から違うんじゃない？と言われた
  - au Pay がスーパアプリって言われると、結構スーパアプリでは？
  - サードパーティなアプリがスーパアプリのプラットフォームに乗っかることができるのが強みな気がするが、ファーストパーティだけにしたら普通の機能が多いアプリと何が違うのかわからん
  - ミニアプリ毎に認証とかしなくて良くなったってのは良いことかも

# 16: セキュリティ:ブラウザを守る HTTP の機能

- サニタイズってもう言わないってこと？
- 文字列結合は要注意は本当にそうだけど、よくやってしまう。。

  - 自作よりもちゃんと提供されているものを使えるようにするのも技術力！

- CSP のベスプラはアプリに作る際に確認すべしな気がする
- Content-Security-Policy-Report-Only ヘッダーでデバッグしながら徐々に設定するのが大事っぽい
- CORS はサーバーからタダノリのようなことを守るってあったけど、本当？

  - タダノリだったら CLI とかブラウザ以外からのリクエストはどうやって守るの？CORS では守れないはず
  - 個人的には CSRF のような悪意のあるサイトからのクロスオリジンな通信を防ぐためな気がしている
    - もしクロスオリジンの許可をしていて、Cookie も許可していたら、認証情報を奪わずとも、ターゲットを悪意のあるサイトまで呼び出せれば、Cookie を使って悪意のあるサイトからリクエストできる気がする
    - ただ、CSRF は CSRF トークンを html に埋め込んで後で検証するのが良いのかな？

- HSTS は大事。サービスに必須でいいと思う。
- Google への申請で初回から HTTPS になるってすごいけど、申請だるそう
- Security Headers めちゃめちゃ便利やん
- プロダクトオーナーからセキュリティ観点のない依頼は断るべきっていうのは大事だし、セキュリティの知識を持っていないときついな〜という感じ
- 様々なパスワードストレッチのアルゴリズムやライブラリがあるんだな〜
- 小規模なアプリなら認証管理は絶対 SaaS 使いたい
- パスワードマネージャーが偽サイトにいることを暗黙的に教えてくれるってのはなるほどと思った
- パスワードマネージャ使うには input の autocomplete 属性を適切に設定しないといけないのは知らなかった
- otpauth っていうスキームで URL っぽいのがあるからここにシークレット情報を入れておいて、QR コードなどから登録できるのか
- シークレットと現在時刻からデータを計算しているっぽい
- totp は時間のズレもパラメータで許容するようにできるっぽい
- PassKeys や WebAuthn はオリジンごとに鍵が発行されているので、フィッシングサイトに強いってのはなるほどと思った
- 3rd パーティ Cookie の話が少しわからなかったけど、広告業者の web サイトが iframe で埋め込まれていると、どのサイトに埋め込まれているのかが Cookie に入れ込んで iframe 経由で送ることができ、広告業者はデータを収集できるってことかな？
  - そして safari とかはそれをブロックしているから、3rd パーティ Cookie による送信が行えないってこと？
- Finger Print という様々な情報から完全ではないがユーザーを特定する方法
  - 日本だと時差がないから特定しにくいっていうのは面白い
- ETag にユーザー ID を入れるってハックすぎない？
- 頑張ってユーザーの行動を取得するのが広告業者にとっては大事ってことかな？
